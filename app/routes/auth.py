...
@users_router.get("/users/profile/create", dependencies=[Depends(has_role(["passenger", "driver"]))])
...
@users_router.post("/users/profile/create", dependencies=[Depends(has_role(["passenger", "driver"]))])
async def create_profile(
    ...
    try:
        if profile_pic and profile_pic.filename:
            ...
            supabase_admin.storage.from_("passengers_profile_pic").upload(file_path, file_content, {"content-type": content_type})
        ...
        raise HTTPException(status_code=status_code, detail=f"File upload failed: {str(e)}")
    ...
    try:
        ...
        new_phone = "0" + phone[3:]
        supabase.table("users").update(user_profile).eq("phone_number", new_phone).execute()
        return {"message": "Profile created successfully!"}
    except Exception as db_err:
        raise HTTPException(status_code=500, detail=f"Database update failed: {str(db_err)}")
    
@users_router.patch("/users/profile/edit", dependencies=[Depends(has_role(["passenger", "driver"]))])
async def edit_profile(
    ...
    if profile_pic and profile_pic.filename:
        ...
        existing_files = supabase_admin.storage.from_("passengers_profile_pic").list(str(user_id))
        if existing_files:
            paths_to_delete = [f"{user_id}/{f['name']}" for f in existing_files]
            try:
                supabase_admin.storage.from_("passengers_profile_pic").remove(paths_to_delete)
            except Exception as del_err:
                print(f"Failed to delete old pics: {del_err}")

        # Upload new pic
        try:
            supabase_admin.storage.from_("passengers_profile_pic").upload(file_path, file_content, {"content-type": content_type, "upsert": "true"})
            image_url = f"{str(url)}/storage/v1/object/public/passengers_profile_pic/{file_path}"
    ...
    try:
        ...
        if not phone:
            raise HTTPException(status_code=400, detail="No phone number on account")
        new_phone = "0" + phone[3:]
        supabase.table("users").update(user_profile_update).eq("phone_number", new_phone).execute()
        return {"message": "Profile updated successfully!"}
    except Exception as db_err:
        raise HTTPException(status_code=500, detail=f"Database update failed: {str(db_err)}")
    

@users_router.post("/users/verify/email", dependencies=[Depends(has_role(["passenger", "driver"]))])
def verify_email(
    user_id: Annotated[str, Depends(get_current_user)],
    email: Annotated[EmailStr, Form()],
    authorization: Annotated[str, Header()]
):
    existing_user = supabase.table("users").select("id").eq("email", str(email)).execute()
    print(existing_user.data)
    if not existing_user.data:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "User not found!")
    
    supabase.table("users").select("id, active_mail").eq("email", email).eq("active_mail", False).execute()
...
@users_router.post("/users/verify/email/otp", dependencies=[Depends(has_role(["passenger", "driver"]))])
def verify_email_otp(
    email: Annotated[EmailStr, Form()],
    otp: Annotated[str, Form()],
    authorization: Annotated[str, Header()]
):
    existing_user = supabase.table("users").select("id, role").eq("email", str(email)).execute()
    if not existing_user.data:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "User not found!")
